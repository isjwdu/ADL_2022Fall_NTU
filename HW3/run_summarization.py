...
class DataTrainingArguments:
    ...
    do_sample: Optional[bool] = field(
        default=False,
        metadata={
            "help": " If set to `False` greedy decoding is used. Otherwise sampling is used. Defaults to `False` as defined in `configuration_utils.PretrainedConfig`."
        },
    )
...
def main():
    ...
    def compute_metrics(eval_preds):
        ...
        result1 = metric(decoded_preds, decoded_labels)
    ...
    if training_args.do_eval:
        ...
        metrics = trainer.evaluate(max_length=max_length, num_beams=num_beams, do_sample=do_sample, top_k=top_k, top_p=top_p, temperature=temperature, metric_key_prefix="eval")
    ...
    if training_args.do_predict:
        ...
        predict_results = trainer.predict(
            predict_dataset, metric_key_prefix="predict", max_length=max_length, num_beams=num_beams, do_sample=do_sample, top_k=top_k, top_p=top_p, temperature=temperature
        )
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                with open(output_prediction_file, "w") as writer:
                    writer.write("\n".join(predictions))
                test_id_list = raw_datasets["test"]["id"]
                
                with open(data_args.output_file, 'w', encoding='utf-8') as fp:
                    for idx, pred in enumerate(predictions):
                        json.dump({'title': pred, 'id': test_id_list[idx]}, fp, ensure_ascii=False)
                        fp.write('\n')
